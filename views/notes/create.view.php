<?php
require_once BASE_PATH.'views/partials/head.php';
require_once BASE_PATH.'views/partials/nav.php';
require_once BASE_PATH.'views/partials/banner.php';
?>
    <main class="text-white font-mono">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 flex flex-col justify-center items-center">
            <div>
                <form method="POST" action="/notes">
                    <div class="space-y-5">
                        <div class="border-b border-white/10 pb-8">
                            <h2 class="text-base/7 font-semibold text-white">Create a note</h2>
                            <p class="mt-1 text-sm/6 text-gray-400">Save your ideas, so you never forget!</p>

                            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                <div class="col-span-full">
                                    <label for="body" class="block text-sm/6 font-medium text-white">Note</label>
                                    <div class="mt-2">
                                        <textarea id="body" name="body" rows="3" autofocus
                                                  class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1
                                                  -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6"><?= $body ?? '' ?></textarea>
                                        <?php if (isset($errors)) : ?>
                                            <p class="text-sm text-red-300 mt-3"><?= $errors['body'] ?></p>
                                        <?php endif; ?>
                                    </div>
                                    <p class="mt-3 text-sm/6 text-gray-400">describe your thoughts...</p>
                                </div>
                            </div>
                        </div>
                        <div class="mt-6 flex items-center justify-end gap-x-6">
                            <button type="reset" class="text-sm/6 font-semibold text-white">Cancel</button>
                            <button type="submit"
                                    class="rounded-md bg-indigo-500 px-3 py-2 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
                                Save
                            </button>
                        </div>
                </form>
            </div>
        </div>
    </main>
<?php
require_once BASE_PATH.'views/partials/foot.php';
?>