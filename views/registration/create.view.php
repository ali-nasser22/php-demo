        ...
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form action="/register" method="POST" class="space-y-6">
                <div>
                    ...
                    <div class="mt-2">
                        <input id="name" type="text" name="name" required value="<?= $values['name'] ?? '' ?>"
                               class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6"/>
                        <?php if (isset($errors['name'])) : ?>
                            <p class="text-sm text-red-300 mt-3"><?= $errors['name'] ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                <div>
                    ...
                    <div class="mt-2">
                        <input id="email" type="email" name="email" required autocomplete="email" value="<?= $values['email'] ?? '' ?>"
                               class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6"/>
                        <?php if (isset($errors['email'])) : ?>
                            <p class="text-sm text-red-300 mt-3"><?= $errors['email'] ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        ...
                        <?php if (isset($errors['password'])) : ?>
                            <p class="text-sm text-red-300 mt-3"><?= $errors['password'] ?></p>
                        <?php endif; ?>
                    </div>
                </div>
